...
with an oscillatory stress-energy electromagnetic source.
"""
...
def prodsup(v,L,R,z,t,k0): #v represents the integration variable
    return (0.5/k0)*v*np.sin(k0*L)*np.ones(np.shape(t))

def ct(v,L,R,z,t,k0):
    return -(0.5/k0**2)*np.cos(k0*(z+v+t))*np.ones(np.shape(R))

def Rplusz(v,L,R,z,t,k0):
    var=R*np.exp(np.arcsinh(v/R))
    (Si,Ci)=sici(k0*var)
    return (0.25/k0**2) * (-np.cos(k0*(z+var)) - (k0*R**2/var)*np.sin(k0*(z+var)) + (k0*R)**2 * (-np.sin(k0*z)*Si + np.cos(k0*z)*Ci) )

def prodinf(v,L,R,z,t,k0):
    return 0*np.ones(np.shape(R))*np.ones(np.shape(t))
