...
associated with the metric deformation generated by a cylinder moving at the speed of light c,
with an oscillatory and a constant stress-energy electromagnetic source.
This source is associated with a pulse of linearly polarized light.
"""
...
"""
ANTIDERIVATIVES (Multiply return by 0 to remove a function for test purposes)
"""

def prodsup(v,L,R,z,t,k0): #v represents the integration variable
    return (0.5/k0)*v*np.sin(k0*L)*np.ones(np.shape(t)) + 0.25*v*(2*L - 2*z-v)

def ct(v,L,R,z,t,k0):
    return -(0.5/k0**2)*np.cos(k0*(z+v+t))*np.ones(np.shape(R)) + 0.5*v*t*np.ones(np.shape(R))

def Rplusz(v,L,R,z,t,k0):
    var=R*np.exp(np.arcsinh(v/R))
    (Si,Ci)=sici(k0*var)
    return (0.25/k0**2) * (-np.cos(k0*(z+var)) - (k0*R**2/var)*np.sin(k0*(z+var))
            + (k0*R)**2 * (-np.sin(k0*z)*Si + np.cos(k0*z)*Ci) ) + 0.25*(v*np.sqrt(R**2 + v**2)+ R**2 * np.arcsinh(v/R))

def prodinf(v,L,R,z,t,k0):
    return 0.25*v*(v + 2*z)*np.ones(np.shape(R))

def zer(v,L,R,z,t,k0):
    return -(0.5/k0)*np.heaviside(-v,0.5)*v*np.sin(k0*z) + (0.25/k0**2)*np.heaviside(v,0.5)*(np.cos(k0*(z + 2*v))-np.cos(k0*z)) - \
            0.25*np.sign(v)*v**2*np.ones(np.shape(R))

"""
COMPLETE SOLUTION
"""

def Solution(L,R,z,t,k0):
...
#-----  np.zeros(np.shape(t),dtype=bool) can be added in some local conditions to negate some parts of the integration (especially useful for studying the effect of a condition on a function)
    ...
    for i,j in enumerate(FoncList):
        Sol+=j(Bornes[i][0],L,R,z,t,k0)-j(Bornes[i][1],L,R,z,t,k0) #Adds to the solution F(Up_bound)-F(Low-bound) for each function.
