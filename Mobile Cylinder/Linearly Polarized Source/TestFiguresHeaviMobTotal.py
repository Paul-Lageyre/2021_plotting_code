...
def tslice(t,N=4,lim=None, lim_inf=None, ratiomin=0.004, ratiomax=100, Npoints=5000):
    ...
    Lmax=np.cbrt(V*ratios[-1]**2 /np.pi)
...
#        if -2*L> Domaine[0] and L/2<Domaine[1]:
        x=np.linspace(-Lmax,Lmax/1.8,Npoints)
...
#        x=np.linspace(-10*L,L/2,100) # other plot setting
        Z= Solution(L,R,x+L/2,t*np.ones(np.shape(x)),2*k0)[0]
...
def tslice_kprop(t, Nk, N=4,lim=None, lim_inf=None, ratiomin=0.004, ratiomax=5000):
    """
...
def xslice(tmax,x0=0,N=4,lim=None,centered=True, ratiomin=0.004, ratiomax=100, Nk=False):
    ...
    for a,i in enumerate(ratios):
        ...
        if Nk:
            k0=2.*np.pi*Nk/L
        if centered:
            Z= np.max(Solution(L,R,X+L/2+x0,Yloc,2*k0)[0],axis=1)
        else:
            Z= np.max(Solution(L,R,X+x0,Yloc,2*k0)[0],axis=1)
...
def xsliceMax(tmax,N=4,lim=None, ratiomin=0.004, ratiomax=100, Nk=False):
    ...
    for a,i in enumerate(ratios):
        ...
        if Nk:
            k0=2.*np.pi*Nk/L
        Z= np.max(Solution(L,R,X,Yloc,2*k0)[0],axis=1)
...
def xsliceP(tmax,x0=0,N=4,lim=None,centered=True, ratiomin=1e-5, ratiomax=0.1, P=1e15):
    ...
    for a,i in enumerate(ratios):
        ...
        R=np.cbrt(V/(r*np.pi))
        if centered:
            Z= P*Solution(L,R,L/2+x0*np.ones(np.shape(yloc)),yloc,2*k0)[0] /R**2
        else:
            Z= P*Solution(L,R,x0*np.ones(np.shape(yloc)),yloc,2*k0)[0]/R**2
...
def xslicePMax(tmax,N=4,lim=None, ratiomin=1e-5, ratiomax=0.1, P=1e15, Nk=False):
    ...
    for a,i in enumerate(ratios):
        ...
        Z= np.max(Solution(L,R,X,Yloc,2*k0)[0],axis=1)
#        Zm= np.min(Solution(L,R,X,Yloc,k0)[0],axis=1)
        line, = plt.semilogx(yloc, Khi*P*Z/R**2 /c)
        if lim:
            line.axes.set_ylim(0,lim)
        line.axes.ticklabel_format(axis='y', style='sci', scilimits=(-2,2))
...
def maxStat(t,rmin,rmax,N):
    ...
    for i in rs:
        ...
        R=np.cbrt(V/(r*np.pi))
        x=np.linspace(-L,L,500)
        Z=np.max(Solution(L,R,x,t*np.ones(np.shape(x)),2*k0)[0])
        Zm=np.min(Solution(L,R,x,t*np.ones(np.shape(x)),2*k0)[0])
...
def maxStatKvar(t,rmin=-4,rmax=4,Npoints=500,wlmin=-1,wlmax=1,N=4):
    ...
    for j in ws:
        ...
        for i in rs:
            ...
            R=np.cbrt(V/(r*np.pi))
            x=np.linspace(-L,L,500)
            Z=np.max(Solution(L,R,x,t*np.ones(np.shape(x)),2*kl)[0])
            Zm=np.min(Solution(L,R,x,t*np.ones(np.shape(x)),2*kl)[0])
            maxlist.append(Khi*I*(l0**2)*Z/c)
            minlist.append(Khi*I*(l0**2)*Zm/c)
...
def maxStat_kpropLvar(t,Lmin,Lmax,N,Nk,R=50):
    ...
    for i in Ls:
        ...
        k0=2.*np.pi*Nk/i
        Z=np.max(Solution(i,R,x,t*np.ones(np.shape(x)),2*k0)[0])
        Zm=np.min(Solution(i,R,x,t*np.ones(np.shape(x)),2*k0)[0])
        maxlist.append(Khi*I*(l0**2)*Z/c)
...
def surfplot(L,R,N,k1,domain=Domaine):
    """
...
def maxPcte(t,N,Rmin=1,Rmax=1e5,Lmin=5,Lmax=5e3,NL=4,P=1e15,lim=None,Nk=False):
    ...
    for a,i in enumerate(Ls):
        ...
        for b,j in enumerate(Rs):
            Iloc=P/(np.pi*j**2)
            Z[b]= Iloc*np.max(Solution(i,j,x,t*np.ones(np.shape(x)),2*k0)[0])
...
#        x=np.linspace(-Le,Le,500)
        ...
        Z[b]= Iloc*np.max(Solution(Le,Re,x,t*np.ones(np.shape(x)),2*k0)[0])
#        Zm[b]= Iloc*np.min(Solution(Le,Re,x,t*np.ones(np.shape(x)),k0)[0])

    if scale=='log':
        line, =plt.loglog(Nks,Khi*Z/c)
    else:
        line, =plt.semilogx(Nks,Khi*Z/c)
        line.axes.ticklabel_format(axis='y', style='sci', scilimits=(-2,2))
